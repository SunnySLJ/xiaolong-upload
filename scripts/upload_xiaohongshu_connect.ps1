...
if (-not $listening) {
    ...
    Start-Process -FilePath $chrome -ArgumentList "--remote-debugging-port=$port", "--user-data-dir=$profileDir", "https://creator.xiaohongshu.com/publish/publish?from=homepage&target=video" -WindowStyle Normal
