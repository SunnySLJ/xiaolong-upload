...
2. macOS 优先使用根入口 `upload.py` 配合 `/opt/homebrew/bin/python3.11`。
...
```bash
/opt/homebrew/bin/python3.11 upload.py --platform kuaishou "/Users/h/Desktop/generated_video.mp4" "记录生活小确幸" "把今天的片段剪成短视频,记录平凡里的小美好。喜欢的话点个赞~" "生活记录,日常分享,美好时光,vlog"
```
...
5. `python3` 版本低于 3.10
   - `nodriver` 无法运行;macOS 上优先改用 `/opt/homebrew/bin/python3.11`。
...
3. 在 macOS 上先确认解释器:
   ...
   - 改用 `/opt/homebrew/bin/python3.11`。
4. 如果普通 profile 模式报 `Failed to connect to browser`:
   ...
   - 再用 connect 模式上传:
   - `AUTH_MODE=connect CDP_DEBUG_PORT=9225 CDP_ENDPOINT=http://127.0.0.1:9225 /opt/homebrew/bin/python3.11 upload.py --platform kuaishou "<视频路径>" "<标题>" "<文案>" "<标签1,标签2,...>"`
...
6. macOS 优先命令:
   - `/opt/homebrew/bin/python3.11 upload.py --platform kuaishou "<视频路径>" "<标题>" "<文案>" "<标签1,标签2,...>"`
7. 成功判定:
   - 必须看到 `视频发布成功`。
8. 上传前先做登录检查:
   - `python scripts/platform_login.py --platform kuaishou --check-only`
...
/opt/homebrew/bin/python3.11 scripts/cleanup_uploaded_videos.py
```
