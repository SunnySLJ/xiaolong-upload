...
用法：
    from api import upload_to_xiaohongshu
    upload_to_xiaohongshu(video_path="...", title="...", description="...", tags=["..."])
"""
import asyncio
import sys
...
def upload_to_xiaohongshu(
    ...
    account_file = str(COOKIES_DIR / f"xiaohongshu_{account_name}.json")
    ...
    async def _do_upload() -> bool:
        ...
        try:
            if browser_tab:
                keep_open = len(browser_tab) > 2 and browser_tab[2]
                ok = await app.upload(
