# 抖音上传统用脚本 - Connect 模式（连接已打开的 Chrome）
# 用法: .\upload_douyin_connect.ps1 "视频路径" "标题" [文案] [标签]
...
if (-not $listening) {
    ...
    Start-Process -FilePath $chrome -ArgumentList "--remote-debugging-port=$port", "--user-data-dir=$profileDir", "https://creator.douyin.com/creator-micro/content/upload" -WindowStyle Normal
    Write-Host "等待 Chrome 就绪..."
    Start-Sleep -Seconds 5
}

# 执行上传（使用 cookie 文件存储登录态，后续只需提供 cookie 无需登录）
