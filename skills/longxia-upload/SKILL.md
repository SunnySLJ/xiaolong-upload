...
- Windows 项目根目录:`C:\Users\爽爽\Desktop\shuang\xiaolong-upload`
- macOS 当前本机项目根目录:`/Users/h/Desktop/xiaolong-upload`
- 若后续把仓库同步到 OpenClaw 工作区,再把命令里的项目根目录替换成真实副本路径;不要继续使用旧机器目录。
...
2. macOS 优先使用根入口 `upload.py` 配合 `/opt/homebrew/bin/python3.12`。
...
9. **上传发布成功后自动关闭浏览器**(2026-03-31 更新),避免浏览器进程积累占用资源。
...
```bash
/opt/homebrew/bin/python3.12 upload.py --platform kuaishou "/Users/h/Desktop/generated_video.mp4" "记录生活小确幸" "把今天的片段剪成短视频，记录平凡里的小美好。喜欢的话点个赞~" "生活记录，日常分享，美好时光，vlog"
```
...
3. 若平台当前无法复用登录:
   - 明确提醒用户需要重新登录
   - 指向 `python scripts/platform_login.py --platform <platform>` 或对应平台 connect 登录流程
...
**⚠️ Python 版本硬性规定(2026-03-31 更新)**:

- **统一使用 `/opt/homebrew/bin/python3.12`**
- 如果项目有 `.venv`,使用 `.venv/bin/python3.12`
- **禁止使用 `python3.12` 或更低版本**
...
5. `python3` 版本低于 3.10
   - `nodriver` 无法运行;macOS 上优先改用 `/opt/homebrew/bin/python3.12`。
...
3. 在 macOS 上先确认解释器:
   ...
   - 改用 `/opt/homebrew/bin/python3.12`。
4. 如果普通 profile 模式报 `Failed to connect to browser`:
   - 手动拉起专用 Chrome:
   - `open -na "Google Chrome" --args --remote-debugging-port=9225 --user-data-dir="/Users/h/Desktop/xiaolong-upload/cookies/chrome_connect_ks" --start-maximized https://cp.kuaishou.com/article/publish/video`
   - 再用 connect 模式上传:
   - `AUTH_MODE=connect CDP_DEBUG_PORT=9225 CDP_ENDPOINT=http://127.0.0.1:9225 /opt/homebrew/bin/python3.12 upload.py --platform kuaishou "<视频路径>" "<标题>" "<文案>" "<标签1,标签2,...>"`
...
6. macOS 优先命令:
   - `/opt/homebrew/bin/python3.12 upload.py --platform kuaishou "<视频路径>" "<标题>" "<文案>" "<标签1,标签2,...>"`
7. 成功判定:
   - 必须看到 `视频发布成功`。
8. 上传前先做登录检查:
   - `python scripts/platform_login.py --platform kuaishou --check-only`
...
- 时间:`2026-03-27`
- 视频:`/Users/h/Desktop/generated_video.mp4`
- 文案:
  - 标题:`记录生活小确幸`
...
**执行命令**:
```bash
cd /Users/h/Desktop/xiaolong-upload
...
```
[INFO] 开始清理视频文件
[INFO] 输出目录:/Users/h/Desktop/xiaolong-upload/flash_longxia/output
[INFO] 保留最近 1 天的文件
