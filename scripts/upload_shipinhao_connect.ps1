# 视频号上传统用脚本 - Connect 模式（连接已打开的 Chrome）
# 用法: .\upload_shipinhao_connect.ps1 "视频路径" "标题" [文案] [标签]
...
if (-not $listening) {
    ...
    Start-Process -FilePath $chrome -ArgumentList "--remote-debugging-port=$port", "--user-data-dir=$profileDir", "--start-maximized", "https://channels.weixin.qq.com/platform/post/create" -WindowStyle Maximized
...
try {
    python upload.py --platform shipinhao $VideoPath $Title $Description $Tags
} finally {
    Pop-Location
}
